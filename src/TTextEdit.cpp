...
QString TTextEdit::getSelectedText(const QChar& newlineChar, const bool showTimestamps)
{
    ...
    int endLine = std::min(mPB.y(), static_cast<int>(mpBuffer->lineBuffer.size() - 1));
    int offset = endLine - startLine;
    int startPos = std::max(0, mPA.x());
    int endPos = std::min(mPB.x(), static_cast<int>(mpBuffer->lineBuffer.at(endLine).size() - 1));
...
void TTextEdit::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key()) {
        ...
        case Qt::Key_Right: {
            ...
            if (mCaretColumn < (mpBuffer->lineBuffer.at(mCaretLine).length() - 1)) {
                if (QGuiApplication::keyboardModifiers().testFlag(Qt::ControlModifier)) {
                    ...
                    nextBoundary = std::min(nextBoundary, static_cast<int>(line.length() - 1));
                    newCaretColumn = nextBoundary;
                    newCaretLine = mCaretLine;
                } else {
                    newCaretColumn = mCaretColumn + 1;
        ...
        case Qt::Key_PageDown:
            newCaretLine = std::min(mCaretLine + mScreenHeight, static_cast<int>(mpBuffer->lineBuffer.length() - 2));
            break;
        case Qt::Key_C:
            if (QGuiApplication::keyboardModifiers().testFlag(Qt::ControlModifier)) {
                if (!QGuiApplication::keyboardModifiers().testFlag(Qt::ShiftModifier)) {
