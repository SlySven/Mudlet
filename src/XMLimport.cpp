    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (name() == qsl("MudletPackage")) {
                ...
                readPackage();
            } else if (!name().compare(QLatin1String("map"))) {
                readMap();
                mpHost->mpMap->audit();
...
std::pair<dlgTriggerEditor::EditorViewType, int> XMLimport::importFromClipboard()
{
    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("MudletPackage"))) {
                result = readPackage();
            } else {
...
void XMLimport::readVariable(TVar* pParent)
{
    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("name"))) {
                keyName = readElementText();
                continue;
            } else if (!name().compare(QLatin1String("value"))) {
                value = readElementText();
                continue;
            } else if (!name().compare(QLatin1String("keyType"))) {
                keyType = readElementText().toInt();
                continue;
            } else if (!name().compare(QLatin1String("valueType"))) {
                ...
                lI->setValue(var);
                continue;
            } else if (!name().compare(QLatin1String("VariableGroup")) || !name().compare(QLatin1String("Variable"))) {
                readVariable(var);
            } else {
...
void XMLimport::readVariablePackage()
{
    ...
    while (!atEnd()) {
        readNext();
        if (isStartElement()) {
            if (!name().compare(QLatin1String("VariableGroup")) || !name().compare(QLatin1String("Variable"))) {
                readVariable(mpVar);
            } else if (!name().compare(QLatin1String("HiddenVariables"))) {
                readHiddenVariables();
            } else {
...
void XMLimport::readMap()
{
    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("areas"))) {
                ...
                readAreas();
            } else if (!name().compare(QLatin1String("rooms"))) {
            ...
            } else if (!name().compare(QLatin1String("environments"))) {
...
void XMLimport::readEnvColors()
{
    while (!atEnd()) {
        ...
        if (!name().compare(QLatin1String("environment"))) {
            readEnvColor();
        }
    }
}
...
void XMLimport::readAreas()
{
    while (!atEnd()) {
        ...
        if (!name().compare(QLatin1String("areas"))) {
            break;
        } else if (!name().compare(QLatin1String("area"))) {
            readArea();
        }
    }
}
...
void XMLimport::readHelpPackage()
{
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("helpURL"))) {
...
void XMLimport::readHostPackage()
{
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("Host"))) {
                readHost(mpHost);
            } else {
...
void XMLimport::readHost(Host* pHost)
{
    ...
    if (attributes().hasAttribute("AmbigousWidthGlyphsToBeWide")) {
        const QString ambiguousWidthSetting(attributes().value(qsl("AmbigousWidthGlyphsToBeWide")).toString());
    ...
    if (attributes().hasAttribute("caretShortcut")) {
        const QString caretShortcut(attributes().value(qsl("caretShortcut")).toString());
    ...
    if (attributes().hasAttribute("blankLineBehaviour")) {
        const QString blankLineBehaviour(attributes().value(qsl("blankLineBehaviour")).toString());
    ...
    QString ignore = attributes().value(qsl("mDoubleClickIgnore")).toString();
    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("name"))) {
            ...
            } else if (!name().compare(QLatin1String("mInstalledModules"))) {
            ...
            } else if (!name().compare(QLatin1String("mInstalledPackages"))) {
                readStringList(pHost->mInstalledPackages, qsl("Host"));
            } else if (!name().compare(QLatin1String("url"))) {
                pHost->mUrl = readElementText();
            } else if (!name().compare(QLatin1String("serverPackageName"))) {
                pHost->mServerGUI_Package_name = readElementText();
            } else if (!name().compare(QLatin1String("serverPackageVersion"))) {
                pHost->mServerGUI_Package_version = readElementText();
            } else if (!name().compare(QLatin1String("port"))) {
                pHost->mPort = readElementText().toInt();
            } else if (!name().compare(QLatin1String("borderTopHeight"))) {
                borders.setTop(readElementText().toInt());
            } else if (!name().compare(QLatin1String("borderBottomHeight"))) {
                borders.setBottom(readElementText().toInt());
            } else if (!name().compare(QLatin1String("borderLeftWidth"))) {
                borders.setLeft(readElementText().toInt());
            } else if (!name().compare(QLatin1String("borderRightWidth"))) {
                borders.setRight(readElementText().toInt());
            } else if (!name().compare(QLatin1String("commandLineMinimumHeight"))) {
                pHost->commandLineMinimumHeight = readElementText().toInt();
            } else if (!name().compare(QLatin1String("wrapAt"))) {
                pHost->mWrapAt = readElementText().toInt();
            } else if (!name().compare(QLatin1String("wrapIndentCount"))) {
                pHost->mWrapIndentCount = readElementText().toInt();
            } else if (!name().compare(QLatin1String("mCommandSeparator"))) {
                pHost->mCommandSeparator = readElementText();
            } else if (!name().compare(QLatin1String("mCommandLineFgColor"))) {
                pHost->mCommandLineFgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mCommandLineBgColor"))) {
                pHost->mCommandLineBgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mFgColor"))) {
                pHost->mFgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBgColor"))) {
                pHost->mBgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mCommandFgColor"))) {
                pHost->mCommandFgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mCommandBgColor"))) {
                pHost->mCommandBgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBlack"))) {
                pHost->mBlack.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightBlack"))) {
                pHost->mLightBlack.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mRed"))) {
                pHost->mRed.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightRed"))) {
                pHost->mLightRed.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBlue"))) {
                pHost->mBlue.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightBlue"))) {
                pHost->mLightBlue.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mGreen"))) {
                pHost->mGreen.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightGreen"))) {
                pHost->mLightGreen.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mYellow"))) {
                pHost->mYellow.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightYellow"))) {
                pHost->mLightYellow.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mCyan"))) {
                pHost->mCyan.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightCyan"))) {
                pHost->mLightCyan.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mMagenta"))) {
                pHost->mMagenta.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightMagenta"))) {
                pHost->mLightMagenta.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mWhite"))) {
                pHost->mWhite.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightWhite"))) {
                pHost->mLightWhite.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mDisplayFont"))) {
            ...
            } else if (!name().compare(QLatin1String("mCommandLineFont"))) {
                pHost->mCommandLineFont.fromString(readElementText());
            } else if (!name().compare(QLatin1String("commandSeperator"))) {
            ...
            } else if (!name().compare(QLatin1String("mFgColor2"))) {
                pHost->mFgColor_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBgColor2"))) {
                pHost->mBgColor_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mRoomBorderColor"))) {
                pHost->mRoomBorderColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mMapInfoBg"))) {
            ...
            } else if (!name().compare(QLatin1String("mBlack2"))) {
                pHost->mBlack_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightBlack2"))) {
                pHost->mLightBlack_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mRed2"))) {
                pHost->mRed_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightRed2"))) {
                pHost->mLightRed_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBlue2"))) {
                pHost->mBlue_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightBlue2"))) {
                pHost->mLightBlue_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mGreen2"))) {
                pHost->mGreen_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightGreen2"))) {
                pHost->mLightGreen_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mYellow2"))) {
                pHost->mYellow_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightYellow2"))) {
                pHost->mLightYellow_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mCyan2"))) {
                pHost->mCyan_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightCyan2"))) {
                pHost->mLightCyan_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mMagenta2"))) {
                pHost->mMagenta_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightMagenta2"))) {
                pHost->mLightMagenta_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mWhite2"))) {
                pHost->mWhite_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mLightWhite2"))) {
                pHost->mLightWhite_2.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mSpellDic"))) {
                pHost->setSpellDic(readElementText());
            } else if (!name().compare(QLatin1String("mLineSize")) || !name().compare(QLatin1String("mRoomSize"))) {
                ...
                // readUnknownElement(...) for "anything not otherwise parsed"
                Q_UNUSED(readElementText());
            } else if (!name().compare(QLatin1String("mMapInfoContributors"))) {
                readLegacyMapInfoContributors();
            } else if (!name().compare(QLatin1String("mapInfoContributor"))) {
                readMapInfoContributor();
            } else if (!name().compare(QLatin1String("profileShortcut"))) {
                readProfileShortcut();
            } else if (!name().compare(QLatin1String("stopwatches"))) {
                readStopWatchMap();
            } else {
...
int XMLimport::readTriggerPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("TriggerGroup")) || !name().compare(QLatin1String("Trigger"))) {
                gotTrigger = true;
...
int XMLimport::readTrigger(TTrigger* pParent)
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            } else if (!name().compare(QLatin1String("script"))) {
            ...
            } else if (!name().compare(QLatin1String("packageName"))) {
                pT->mPackageName = readElementText();
            } else if (!name().compare(QLatin1String("triggerType"))) {
                pT->mTriggerType = readElementText().toInt();
            } else if (!name().compare(QLatin1String("conditonLineDelta"))) {
                pT->mConditionLineDelta = readElementText().toInt();
            } else if (!name().compare(QLatin1String("mStayOpen"))) {
                pT->mStayOpen = readElementText().toInt();
            } else if (!name().compare(QLatin1String("mCommand"))) {
                pT->mCommand = readElementText();
            } else if (!name().compare(QLatin1String("mFgColor"))) {
                pT->mFgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mBgColor"))) {
                pT->mBgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("colorTriggerFgColor"))) {
                pT->mColorTriggerFgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("colorTriggerBgColor"))) {
                pT->mColorTriggerBgColor.setNamedColor(readElementText());
            } else if (!name().compare(QLatin1String("mSoundFile"))) {
                pT->mSoundFile = readElementText();
            } else if (!name().compare(QLatin1String("regexCodeList"))) {
            ...
            } else if (!name().compare(QLatin1String("regexCodePropertyList"))) {
            ...
            } else if (!name().compare(QLatin1String("TriggerGroup")) || !name().compare(QLatin1String("Trigger"))) {
                readTrigger(pT);
            } else {
                readUnknownElement(what);
            }
...
int XMLimport::readTimerPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("TimerGroup")) || !name().compare(QLatin1String("Timer"))) {
                gotTimer = true;
...
int XMLimport::readTimer(TTimer* pParent)
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            } else if (!name().compare(QLatin1String("command"))) {
                pT->mCommand = readElementText();
            } else if (!name().compare(QLatin1String("time"))) {
                pT->setTime(QTime::fromString(readElementText(), "hh:mm:ss.zzz"));
            } else if (!name().compare(QLatin1String("TimerGroup")) || !name().compare(QLatin1String("Timer"))) {
                readTimer(pT);
            } else {
                readUnknownElement(what);
            }
...
int XMLimport::readAliasPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("AliasGroup")) || !name().compare(QLatin1String("Alias"))) {
                gotAlias = true;
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("name"))) {
                ...
                pT->mCommand = readElementText();
            } else if (!name().compare(QLatin1String("regex"))) {
                pT->setRegexCode(readElementText());
            } else if (!name().compare(QLatin1String("AliasGroup")) || !name().compare(QLatin1String("Alias"))) {
                readAlias(pT);
            } else {
                readUnknownElement(what);
            }
...
int XMLimport::readActionPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("ActionGroup")) || !name().compare(QLatin1String("Action"))) {
                gotAction = true;
...
int XMLimport::readAction(TAction* pParent)
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            } else if (!name().compare(QLatin1String("css"))) {
                pT->css = readElementText();
            } else if (!name().compare(QLatin1String("commandButtonUp"))) {
                pT->mCommandButtonUp = readElementText();
            } else if (!name().compare(QLatin1String("commandButtonDown"))) {
                pT->mCommandButtonDown = readElementText();
            } else if (!name().compare(QLatin1String("icon"))) {
                pT->mIcon = readElementText();
            } else if (!name().compare(QLatin1String("orientation"))) {
                pT->mOrientation = readElementText().toInt();
            } else if (!name().compare(QLatin1String("location"))) {
                pT->mLocation = readElementText().toInt();
            } else if (!name().compare(QLatin1String("buttonRotation"))) {
                pT->mButtonRotation = readElementText().toInt();
            } else if (!name().compare(QLatin1String("sizeX"))) {
                pT->mSizeX = readElementText().toInt();
            } else if (!name().compare(QLatin1String("sizeY"))) {
                pT->mSizeY = readElementText().toInt();
            } else if (!name().compare(QLatin1String("mButtonState"))) {
            ...
            } else if (!name().compare(QLatin1String("buttonColor"))) {
                // Not longer present/used, skip over it if it is still in file:
                skipCurrentElement();
            } else if (!name().compare(QLatin1String("buttonColumn"))) {
                pT->mButtonColumns = readElementText().toInt();
            } else if (!name().compare(QLatin1String("posX"))) {
                pT->mPosX = readElementText().toInt();
            } else if (!name().compare(QLatin1String("posY"))) {
                pT->mPosY = readElementText().toInt();
            } else if (!name().compare(QLatin1String("ActionGroup")) || !name().compare(QLatin1String("Action"))) {
                readAction(pT);
            } else {
                readUnknownElement(what);
            }
...
int XMLimport::readScriptPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("ScriptGroup")) || !name().compare(QLatin1String("Script"))) {
                gotScript = true;
...
int XMLimport::readScript(TScript* pParent)
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            } else if (!name().compare(QLatin1String("eventHandlerList"))) {
                readStringList(script->mEventHandlerList, what);
                script->setEventHandlerList(script->mEventHandlerList);
            } else if (!name().compare(QLatin1String("ScriptGroup")) || !name().compare(QLatin1String("Script"))) {
                readScript(script);
            } else {
                readUnknownElement(what);
            }
...
int XMLimport::readKeyPackage()
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("KeyGroup")) || !name().compare(QLatin1String("Key"))) {
                gotKey = true;
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("name"))) {
                ...
                pT->mCommand = readElementText();
            } else if (!name().compare(QLatin1String("keyCode"))) {
                pT->setKeyCode(readElementText().toInt());
            } else if (!name().compare(QLatin1String("keyModifier"))) {
                pT->setKeyModifiers(readElementText().toInt());
            } else if (!name().compare(QLatin1String("KeyGroup")) || !name().compare(QLatin1String("Key"))) {
                readKey(pT);
            } else {
                readUnknownElement(what);
            }
...
void XMLimport::readModulesDetailsMap(QMap<QString, QStringList>& map)
{
    ...
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("key"))) {
                key = readElementText();
            } else if (!name().compare(QLatin1String("filepath"))) {
                entry << readElementText();
            } else if (!name().compare(QLatin1String("zipSync"))) {
                entry << readElementText();
            } else if (!name().compare(QLatin1String("globalSave"))) {
                if (entry.size() < 2) {
                    entry << readElementText();
                } else {
                    skipCurrentElement();
                }
            } else if (!name().compare(QLatin1String("priority"))) {
...
void XMLimport::readStringList(QStringList& list, const QString& whatIsParent)
{
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("string"))) {
...
void XMLimport::readIntegerList(QList<int>& list, const QString& parentName, const QString& whatIsParent)
{
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("integer"))) {
...
void XMLimport::readStopWatchMap()
{
    while (!atEnd()) {
        ...
        if (isEndElement()) {
            ...
            if (!name().compare(QLatin1String("stopwatch"))) {
...
void XMLimport::readLegacyMapInfoContributors()
{
    while (!atEnd()) {
        ...
        if (isStartElement()) {
            if (!name().compare(QLatin1String("mapInfoContributor"))) {
                mpHost->mMapInfoContributors.insert(readElementText());
            }
        }
    }
